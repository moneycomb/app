Meteor.methods({
  ...
  updateSpendStatistics: function (userId) {
    ...
    if (userId == undefined) {
      USER = Meteor.userId();
      console.log(`Using ${Meteor.userId()} as the implicit user...`);
    } else {
      USER = userId;
      console.log(`Using ${USER} as specified as the user`);
    }


    // log.info(Meteor.user().primaryemail() + ": SERVER METHOD: Update Spend Statistics Start");
    ...
    var account = {};
    var FA = FinAccounts.find({userId: USER});
    ...
    let numIgnored = Transactions.find({userId: USER, ignore: true}).count();
    ...
    cursor.forEach(function (transaction) {
      ...
      if (transaction.matchCheck === true) transactionCounts.matchCheck++;
    ...
    // log.info(Meteor.user().primaryemail() + ': SERVER METHOD: Update Spend Statistics END. Elapsed time=' + time);

    // Now let's save the analysis - either insert or update

    let An = Analysis.findOne({userId: USER})
    if (An == undefined) {
      console.log(`Creating Analysis record for ${USER}`)
      var record = {
        userId: USER,
        updatedAt: new Date(),
      ...
      console.log(record);
      Analysis.insert(record);

    } else {

      console.log(`Updating Analysis record for ${USER}`)

      Analysis.update({userId: USER},
        {
          $set: {
            updatedAt: new Date(),
            transactionCounts: transactionCounts,
